"""Cross-family k-curve sweep: does the 'optimal evidence density' generalize?

Tests 4 additional model variants on the same 30 LongMemEval items + top-10 retrieval
from the hybrid run, across k = {3, 5, 8, 10}.

New models:
  - meta-llama/llama-3.2-3b-instruct  (3B class)
  - meta-llama/llama-3.1-8b-instruct  (8B class)
  - qwen/qwen-2.5-7b-instruct         (7B class, no 3B variant available)
  - mistralai/ministral-3b-2512       (3B class, Mistral family)

Compared against (already measured):
  - ibm-granite/granite-4.0-h-micro   (Q4 / ~3B Granite)
  - ibm-granite/granite-4.1-8b        (Q8 / 8B Granite)

Hypothesis from the user: each model has an OPTIMAL evidence density k*.
Granite micro peaks at k=3, Granite 8B at k=10. Does this generalize?
"""
...
_OUT = _HERE / "results" / "minimaltest_model_sweep"
...
def main():
    ...
    print(f"  {len(items)} hybrid items", flush=True)
