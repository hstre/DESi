"""v4.8 — read-only/import-only invariants + historical pins."""
...
def test_no_runtime_module_imports_content_audit_probe() -> None:
    ...
    pattern = re.compile(r"content_audit_probe")
    for sub in forbidden:
        ...
        for p in sub_path.rglob("*.py"):
            ...
            assert not pattern.search(text), (
                f"{p} mentions content_audit_probe"
            )


def test_v48_does_not_edit_forbidden_runtime_files() -> None:
    forbidden_roots = (
    ...
    for root in forbidden_roots:
        ...
        for p in root.rglob("*.py"):
            text = p.read_text(encoding="utf-8")
            assert "content_audit_probe" not in text, p
...
def test_v40_to_v47_historical_artifacts_preserved() -> None:
