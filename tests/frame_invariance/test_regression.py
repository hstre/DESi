"""v3.5 regression — runtime untouched, every prior hash stable."""
...
def test_v34_frame_benchmark_still_40_of_40() -> None:
    """v3.4 frame benchmark must remain 40/40 after v3.5 lands."""
    m = compute_frame_metrics(FrameBenchmarkRunner().run())
    assert m.fully_correct == 40
    assert m.total == 40


def test_no_runtime_module_imports_frame_invariance() -> None:
    """v3.5 is read-only — no production package may depend on
    src/desi/frame_invariance/."""
    ...
    forbidden = (
        ...
        "self_audit", "doc_anchors", "frames",
        "frame_benchmark",
    )
    pattern = re.compile(r"frame_invariance")
    for sub in forbidden:
        for p in (root / sub).rglob("*.py"):
            text = p.read_text(encoding="utf-8")
            assert not pattern.search(text), (
                f"{p} mentions frame_invariance — directive violation"
            )
