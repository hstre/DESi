"""v4.9 — report build + recommendation + artifact pin."""
...
def _build():
    ...
    return build_v49_report(started_at=when, finished_at=when)
...
def test_pre_v49_hashes_referenced() -> None:
    r = _build()
    ...
    assert r.v48_replay_hash == V48_REPLAY_HASH
...
def test_artifact_matches_built_report() -> None:
    artifact = _REPO_ROOT / "artifacts" / "v4_9" / "report.json"
    assert artifact.exists()
    data = json.loads(artifact.read_text(encoding="utf-8"))
    r = _build()
    assert data["replay_hash"] == r.replay_hash
    assert data["recommended_next"] == r.recommended_next
    assert data["effect"]["false_support_after"] == 0
    assert data["effect"]["reduction"] == 9
