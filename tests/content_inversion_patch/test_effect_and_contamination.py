"""v4.9 — effect + contamination + non-target invariants."""
...
def test_contamination_is_zero() -> None:
    ...
    assert rep.c1_contamination_count == 0
    assert rep.c2_contamination_count == 0
    assert rep.protected_pool_size > 0


def test_effect_meets_target() -> None:
    e = effect_measure()
    assert e.false_support_before == TARGET_BEFORE_COUNT == 9
    assert e.false_support_after == TARGET_AFTER_COUNT == 0
    assert e.reduction == EXPECTED_REDUCTION == 9


def test_both_target_clusters_fully_retired() -> None:
    e = effect_measure()
