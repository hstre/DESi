"""v3.6 regression — runtime untouched, all prior hashes stable."""
...
def test_v35_frame_invariance_still_400_cases() -> None:
    run = FrameInvarianceRunner().run()
    assert len(run.results) == 400


def test_no_runtime_module_imports_frame_failure_audit() -> None:
    ...
    pattern = re.compile(r"frame_failure_audit")
    for sub in forbidden:
        for p in (root / sub).rglob("*.py"):
            text = p.read_text(encoding="utf-8")
            assert not pattern.search(text), (
                f"{p} mentions frame_failure_audit — directive violation"
            )
