"""v4.6 — read-only/import-only invariants + historical pins."""
...
def test_no_runtime_module_imports_warrant_probe() -> None:
    ...
    pattern = re.compile(r"warrant_probe")
    for sub in forbidden:
        ...
        for p in sub_path.rglob("*.py"):
            ...
            assert not pattern.search(text), (
                f"{p} mentions warrant_probe"
            )


def test_v46_does_not_edit_forbidden_runtime_files() -> None:
    forbidden_roots = (
    ...
    for root in forbidden_roots:
        ...
        for p in root.rglob("*.py"):
            text = p.read_text(encoding="utf-8")
            assert "warrant_probe" not in text, p
...
def test_v40_to_v45_historical_artifacts_preserved() -> None:
