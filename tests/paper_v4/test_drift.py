"""Aufgaben 4 + 7 — drift checks.

Every claim in ``v4_claims.json`` is verified against the
live artifact: the artifact's ``replay_hash`` must match the
claim's ``replay_hash``, and the value at ``field_path``
...
def test_at_least_120_claims() -> None:
    claims = load_claims()
    assert len(claims) >= 120, (
        f"v4.10 requires >= 120 claims, found {len(claims)}"
    )
...
def test_missing_pin_count_is_zero() -> None:
    """Aufgabe 7 — every claim must reference an existing
    artifact file (no missing pins)."""
    missing = 0
    for c in load_claims():
        try:
            load_artifact(c["artifact"])
        except FileNotFoundError:
            missing += 1
    assert missing == 0, f"missing_pin_count={missing}"
