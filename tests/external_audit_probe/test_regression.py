"""v4.2 — read-only/import-only invariants, prior-hash pins."""
...
def test_no_runtime_module_imports_external_audit_probe() -> None:
    ...
    pattern = re.compile(r"external_audit_probe")
    for sub in forbidden:
        ...
        for p in sub_path.rglob("*.py"):
            ...
            assert not pattern.search(text), (
                f"{p} mentions external_audit_probe"
            )


def test_v42_does_not_edit_forbidden_runtime_files() -> None:
    forbidden_roots = (
    ...
    for root in forbidden_roots:
        ...
        for p in root.rglob("*.py"):
            text = p.read_text(encoding="utf-8")
            assert "external_audit_probe" not in text, (
                f"{p} mentions external_audit_probe"
            )
