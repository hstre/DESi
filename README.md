...
**Reproduction.** The dataset is committed at `data/driftbench/driftbench_compression.jsonl` (1,525 rows, one JSON object per trajectory). Every number in the table above is regenerated directly from it — no network, no LLM calls, no PRNG — via `python scripts/reproduce_driftbench.py`, and pinned by `tests/driftbench/test_compression_repro.py` (96.5% compression, correlations 0.438 / 0.466, ρ = 1.06, and 1525/1525 on each preservation flag). Because both correlations are measured on the same trajectories and share the severity variable, the script also runs Steiger’s (1980) Z-test for dependent correlations (closed-form, deterministic): Z = 1.65, p = 0.098 two-sided. ρ’s deviation from 1.0 is therefore **not** statistically significant on this sample — the supported claim is *signal preserved* (no measurable degradation under ~96% compression); the slight sharpening (ρ > 1) is a point estimate, not a demonstrated improvement. What is *not* yet committed to this repository is the upstream pipeline that derives `desi_state_tokens` and `desi_drift` per raw trajectory; those columns are taken as given here, so this reproduces the reported aggregates, not the per-trajectory state extraction.
