# A Read-Only Governance Layer for In-Context Exploration: A Small Synthetic Follow-Up Study
...
A read-only governance layer reads trajectories, classifies them by structure (not reward), assigns soft priority weights and never deletes or pins a trajectory. A second configuration adds a generator agent whose aggressive trajectories the governor scores. The layer is complementary and read-only; it neither learns nor optimises a reward.
