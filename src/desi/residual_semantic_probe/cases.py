...
Selects the chains in the v4.0 corpus that *still* audit as
``LOGICALLY_SUPPORTED`` under ``CAUSAL_CHAIN`` after the v4.3
patch. The expected count is 24 (v4.2 localised 143; v4.3
retired 119 via marker extension; the remaining 24 are the
v4.4 input).

No new chains are constructed here — the case set is a strict
subset of the v4.0 external corpus.
"""
...
def collect_residue_cases() -> tuple[ResidueCase, ...]:
    """Deterministic, in corpus order. Uses the *patched*
    auditor (the same module the v4.3 commit edited)."""
    auditor = LogicalAuditor()
    cases: list[ResidueCase] = []
    for c in all_chains():
