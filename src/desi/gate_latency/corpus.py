"""Aufgabe 3 — corpus reuses v3.22 base + adds v3.21 + v3.22
perspective duplicates to clear the 3000-transition floor."""
...
def _v321_v322_perspectives() -> tuple[ChainEntry, ...]:
    """Perspective duplicates inherited from the v3.21 gate-
    ablation and v3.22 order-probe corpora."""
    out: list[ChainEntry] = []
    for c in ALL_MULTISTEP_CASES:
        out.append(ChainEntry(
    ...
    for c in ALL_ADVERSARIAL_CASES:
        out.append(ChainEntry(
            chain_id=f"v322-adv:{c.case_id}",
            source="v322_adversarial_view",
            text=c.text, is_attack=True,
        ))
    return tuple(out)
