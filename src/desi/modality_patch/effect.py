...
Compares the v4.6 frozen residue (19 cases) with the live
post-patch residue under the v4.7-patched auditor. Reports
overall reduction, per-warrant-class delta, and the
...
_V46_ARTIFACT = _REPO_ROOT / "artifacts" / "v4_6" / "report.json"
...
def measure() -> EffectReport:
    v46 = json.loads(_V46_ARTIFACT.read_text(encoding="utf-8"))
    by_id = {
        c["chain_id"]: c["failure_class"]
        for c in v46["classifications"]
    }
