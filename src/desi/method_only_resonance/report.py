"""v3.58 — method-only resonance report.

Pflichtmetriken (directive § v3.58):

* ``method_pair_resonance``       — global plateau
  resonant_pair_count in method space
* ``method_pair_transfer_rate``   — fraction of
  eligible corpora whose plateau resonance exceeds
  control in method space
* ``method_subadditivity_score``  — global plateau
  subadditivity in method space
* ``method_control_pairs``        — global control
  resonant_pair_count
* ``replay_stability``            — deterministic
  two-run equality

Stop rule: ``method_pair_transfer_rate == 0`` →
method-only resonance is corpus-local
(documentation-only, not a halt; the directive's
"Wenn ... = 0: Method resonance falsifiziert" is a
finding to record, not a sprint stop).
"""
...
from .probe import (
    ...
    MethodPairSummary, eligible_corpora,
...
class V358Report:
    ...
    method_pair_resonance: int
    method_control_pairs: int
    method_subadditivity_score: float
    method_pair_transfer_rate: float
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "method_pair_resonance":
                self.method_pair_resonance,
            "method_control_pairs":
                self.method_control_pairs,
            "method_subadditivity_score":
                self.method_subadditivity_score,
            "method_pair_transfer_rate":
                self.method_pair_transfer_rate,
            "replay_stability":
...
def build_report() -> V358Report:
    ...
    if halt:
        ...
        verdict = "METHOD_RESONANCE_TRANSFERS"
    elif global_plat.resonant_pair_count > (
        global_ctrl.resonant_pair_count
    ):
        verdict = "METHOD_RESONANCE_GLOBAL_ONLY"
    else:
        verdict = "METHOD_RESONANCE_FALSIFIED"

    rationale = (
        f"INFO: probe_radius {METHOD_PROBE_RADIUS}",
        ...
        f"method_pair_transfer_rate {rate} > 0",
        f"{'PASS' if replay == 1.0 else 'FAIL'}: "
        f"replay_stability {replay}",
    )

    return V358Report(
        probe_radius=METHOD_PROBE_RADIUS,
        ...
        method_pair_transfer_rate=rate,
...
def build_method_resonance_artifact(
) -> dict[str, object]:
    plat, ctrl = _gather_per_corpus()
    return {
        "schema_version":
            "v3_58_method_only_resonance",
        "probe_radius": METHOD_PROBE_RADIUS,
        "global_plateau":
