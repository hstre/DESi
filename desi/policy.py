...
def classify(query: str) -> str:
    ...
    A heuristic, not a model. Returns one of: math_arithmetic, code_audit,
    scientific_claim, memory_recall, general. Callers may override.
    """
...
def _score(m: ModelSpec, task_class: str) -> float | None:
    return m.task_scores.get(task_class)
...
def decide(
    ...
    why.append(f"score={s if s is not None else 'unmeasured'} vs target {constraints.accuracy_target}")
    ...
    return Decision(
        ...
        extras={"provider": p.name, "model_id": m.id},
    )
