# Fact-verification (SciFact/FEVER-style) — pietrolesci/nli_fever / ibm-granite/granite-4.1-8b
...
**SciFact source note:** canonical `allenai/scifact` is a deprecated dataset SCRIPT and does not load under datasets>=4; `BeIR/scifact` has no verification labels. The real evidence-style source used here is `pietrolesci/nli_fever` (real claim+evidence+3-class labels; no labels invented).

| field | value |
| --- | --- |
| dataset | `pietrolesci/nli_fever` (split `dev`) |
...
| **exact 3-class accuracy** | **0.570** (of answered) |
| elapsed | 28.87s |
| est. cost | $0.000936 |
